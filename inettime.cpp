...
//
// It compiles and runs under Windows, Linux RedHat and  Ubuntu
// MacOS, OpenVMS 7.2 and FreeBSD 4.8.
// 
//
...
//   - Switched to use snprintf() to reduce the number of compiler warnings.
//   - Accept "Date:" and "date:" in HTTP response header
//   - Removed some code that was left for parsing HTML from usnavy
...
int stime(const time_t* newtime) {
	...
	snprintf(szCmd, DIM(szCmd), "SET TIME=%02d-%s-%04d:%02d:%02d:%02d"
...
////////////////////////////////////////////////////////////////////////////
//
int FormatRequest(char* pszBuffer, int bufSize) {
	int		cch;
	if (gfUseProxy) {
		cch = snprintf(pszBuffer, bufSize
			, "HEAD %s%s HTTP/1.0\r\n"
			"Accept: text/*\r\n"
...
#else
	cchDate = snprintf(pszDate, cchDate, "%04d-%02d-%02d %02d:%02d:%02d"
...
void AdjustClock(SYSTEMTIME& st) {
	...
	if (!SetSystemTime(&st)) {
		...
		snprintf(szCur, DIM(szCur), "%d", GetLastError());
...
void RegEventSource(void) {
	...
	snprintf(szKey, DIM(szKey), "SYSTEM\\CurrentControlSet\\Services\\Eventlog\\Application\\%s", STR_EVENTSOURCE);
					...
					if (toupper(*(argv[n] + 1)) == 'V')
						...
						if (toupper(*(argv[n] + 1)) == 'S')
							...
							if (*(argv[1] + 1) == '?' || *(argv[1] + 1) == 'h') {
								printf("inettime version 1.06 for %s - RedBaronOfAzure. Copyright %s 2018\n\n"
